...
class ObjectGear(bpy.types.Operator):
    ...
    TeethNs = bpy.props.IntProperty(name="Teeth Number", default=20, min=9, soft_max=100)
    ...
    def execute(self,  context):
        ...
        if (prop == "rac"):
            ...
            bpy.ops.mesh.spin(steps=TeethN, dupli=True, angle=2*pi, center=bpy.context.scene.cursor_location,
                              axis=(0.0, 0.0, 1.0))
        ...
        if Pinion is True:
                ...
                if (prop == "nor" or prop == "con"):
                    pcenter = (obj.object.location[0]+Rp+Rg, obj.object.location[1], obj.object.location[2])
                if (prop == "cro"):
                    pcenter = (obj.object.location[0]+Rg-Rp, obj.object.location[1], obj.object.location[2])
                if (prop == "rac"):
                    pcenter = (obj.object.location[0]+Rp, obj.object.location[1], obj.object.location[2])
                objp.object.location = pcenter
                bpy.ops.object.mode_set(mode='EDIT')
                bpy.ops.mesh.spin(steps=TeethN,  dupli=True, angle=2*pi, center=pcenter,  axis=paxis)
                bpy.ops.mesh.delete(type='VERT')
                bpy.ops.object.mode_set(mode='OBJECT')
                if (prop == "con"):
                    bpy.ops.transform.rotate(value=-self.Alphas*pi/180.0, axis=(0, 1, 0))
                    objp.object.location[0] = obj.object.location[0] + Rg
                    objp.object.location[2] = obj.object.location[2] + Rp
        return {'FINISHED'}
...
def menu_func(self,   context):
        layout = self.layout
        layout.operator_context = 'INVOKE_REGION_WIN'
        layout.menu("INFO_MT_mesh_gear_add",   text="Gear",   icon="PLUGIN")
